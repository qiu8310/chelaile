define(['libs/utils'], function(utils) {
  ...
  function positionIn(last) {
    ...
    if (last.top >= offset.top - gap && last.left >= offset.left - gap
      && last.top + last.h <= offset.top + h + gap && last.left + last.w <= offset.left + w + gap) {
      return true;
    } else {
      return false;
    }
  ...
  function touchmove(e) {
    ...
    var left = last.left + touch.clientX - last.x,
      top = last.top + touch.clientY - last.y;
