...
var empty = function() {}, //rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
  scriptTypeRE = /^(?:text|application)\/javascript/i, xmlTypeRE = /^(?:text|application)\/xml/i, jsonType = 'application/json', htmlType = 'text/html', blankRE = /^\s*$/,
...
function ajax(options) {
  ...
  if (settings.data && (typeof settings.data) !== 'string' && utils.toString(settings.data) !== '[object Object]') {
    isXHR2SendType = true;
  } else {
  ...
  if (settings.cache === false) {
    settings.url = utils.appendQuery(settings.url, '_=' + Date.now());
  }
