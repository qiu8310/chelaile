define(['libs/utils'], function(utils) {
  ...
  function Lottery(container, opts) {
    ...
    if (!container || !container.nodeName) throw new Error('Lottery container not exist');
    ...
    var canvas = this.canvas, ctx = this.ctx,
      canvasWidth = opts.width * 2,
      canvasHeight = opts.height * 2;
    var ROWS = {};
    var MARK_COUNT = 0;
    var MARK_TOTAL = Math.round((canvasWidth/opts.strokeWidth) *
        (canvasHeight/opts.strokeWidth));
    ...
    function mousedownHandler(e) {
      ...
      if (!self.isStarted) {
        if (opts.error) opts.error.call(self, 101, '资源尚未加载完成');
        return ;
      }
