...
var Event = require('./event'), Wechat = require('./wechat');
var STATUS = {
  STOPPED: 1,
  PLAYING: 2
};
...
function WechatVoice(appId, localId, opts) {
  ...
  if (!localId || !appId) {
    throw new Error('Absence arguments for new Voice()');
  }
...
WechatVoice.prototype = {
  upload   : function(success, error) {
    ...
    Wechat.uploadVoice({appId: self.appId, localId: self.localId}, {
      success: function(data) {
        ...
        if (success) {
          success.call(self, data.serverId);
        }
      },
      error  : function(res) {
        if (error) {
        ...
        self.trigger('error', [
          {type: 'uploadError', msg: res.err_msg}
        ]);
      }
    });
  },
  play     : function() {
    ...
    Wechat.playVoice({appId: this.appId, localId: this.localId}, {
      ...
      error   : function(res) {
        self.trigger('error', [
          {type: 'playError', msg: res.err_msg}
        ], self);
      }
    });
  },
  stop     : function() {
    ...
    Wechat.stopVoice({appId: this.appId, localId: this.localId}, {
      ...
      error   : function(res) {
        self.trigger('error', [
          {type: 'stopError', msg: res.err_msg}
        ], self);
      }
    });
  },
